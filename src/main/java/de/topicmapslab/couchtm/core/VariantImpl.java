...
/**
 * {@link IVariant} implementation.
 * 
 * @author Hans-Henning Koch (phi04bib[at]studserv.uni-leipzig.de)
 */
public class VariantImpl extends ConstructImpl implements IVariant, IScoped, IReifiable{
	
	private IName parent;
	private String value;
	private Set<ITopic> scope;
	private ITopic reifier;
	private Locator datatype;
	private boolean loaded;
	private IVariant mergedIn;

	public VariantImpl(String id, ITopicMap tm) {
		super(id, tm, IConstant.VARIANT);
		scope = CollectionFactory.createSet();
		loaded = false;
	}
	
	public VariantImpl(String id, String rev, ITopicMap tm, Set<Locator> iids, IName parent, String value, ITopic reifier, Set<ITopic> scope, Locator datatype) {
		super(id, tm, IConstant.VARIANT, parent, iids, rev);
		this.parent = parent;
		this.value = value;
		this.scope = scope;
	...
	public VariantImpl(String id, ITopicMap tm, IName parent, String value, Locator datatype, Set<ITopic> scope) {
		super(id, tm, IConstant.VARIANT, parent, true);
		this.value = value;
		this.datatype = datatype;
		this.scope = scope;
		this.parent = parent;
	...
	public VariantImpl(IVariant variant) {
		super(variant);
		parent = (IName) variant.getParent();
		value = variant.getValue();
		scope = CollectionFactory.createSet();
		scope.addAll(Converter.setTopicToITopic(variant.getScope()));
		reifier = (ITopic) variant.getReifier();
		datatype = variant.getDatatype();
		loaded = variant.getLoaded();
		mergedIn = variant.getMergedIn();
	}
	...
	public void mergeWith(IConstruct construct) {
		...
		if(reifier == null && ((IVariant) construct).getReifier() != null) {
			Topic tmpReifier = ((IVariant) construct).getReifier();
			((IVariant) construct).setReifier(null);
			setReifier(tmpReifier); 
		}
		else if(reifier != null && ((IVariant) construct).getReifier() != null) {
			try {
				MergeUtils.mergeTopics(reifier, (ITopic) ((IVariant) construct).getReifier(), true);
			} catch(Exception e) {
				e.printStackTrace();
			}
		}
	 ...
	 */
	@Override
	public void setMergedIn(IConstruct variant) {
		if(mergedIn != null) {
			mergedIn.setMergedIn(variant);
			return;
		}
		super.setMergedIn(variant);
		mergedIn = (IVariant) variant;
	}
	
	/*
	 * (non-Javadoc)
	 * @see de.topicmapslab.couchtm.internal.api.IVariant#getMergedIn()
	 */
	@Override
	public IVariant getMergedIn() {
	...
	/*
	 * (non-Javadoc)
	 * @see de.topicmapslab.couchtm.internal.api.IVariant#setReifier(org.tmapi.core.Topic, boolean)
	 */
	@Override
	...
	protected void load() {
		if(loaded) return;
		IVariant variant = tmom.getVariant(id);
		if(parent == null) parent = (IName) variant.getParent();
		if(value == null) value = variant.getValue();
		if(reifier == null) reifier = (ITopic) variant.getReifier();
		if(datatype == null) datatype = variant.getDatatype();
		if(scope == null || scope.size() < 1) scope = Converter.setTopicToITopic(variant.getScope());
		loaded = true;
		super.load(variant);
	...
	public JSONObject asJSONObject() {
		...
		JSONObject variant = super.asJSONObject();
		try {
			if(value != null) variant.put("value", value);
			if(scope != null && scope.size() > 0) {
				JSONArray topics = new JSONArray();
				for(Topic topic : scope) topics.put(topic.getId());
				variant.put("scope", topics);
			}
			if(reifier != null) variant.put("reifier", reifier.getId());
			if(datatype != null) variant.put("datatype", datatype.getReference());
	...
	public void remove(boolean tmClear) {
		...
		if(!tmClear) {
			if(parent != null) parent.removeVariant(this);
			if(reifier != null) {
	...
	/*
	 * (non-Javadoc)
	 * @see de.topicmapslab.couchtm.internal.api.IVariant#setParent(de.topicmapslab.couchtm.internal.api.IName)
	 */
	@Override
	public void setParent(IName parent) {
		setParent(parent, false);
	}
	
	/*
	 * (non-Javadoc)
	 * @see de.topicmapslab.couchtm.internal.api.IVariant#setParent(de.topicmapslab.couchtm.internal.api.IName, boolean)
	 */
	@Override
	public void setParent(IName parent, boolean flag) {
		if(!loaded) load();
		if(mergedIn != null) {
			mergedIn.setParent(parent);
			return;
		}
		IName oldParent = parent;
		this.parent = parent;
